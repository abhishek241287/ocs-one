...
| OBS-CW02-003 — no `idx_cells_created_at`; list `ORDER BY created_at DESC` does a top-N heapsort (0.20 ms / 32 kB at 1,058 rows — negligible). Recommend index for consistency with `DEF-CW01-M05-001`. Module-specific. — **CTO ruling 2026-06-28: deferred Low optimization, NOT implemented during CW-02 → MEB-002** | **Closed** (→ MEB-002) |
| OBS-CW02-M06-001 (**Low, Platform**) — SS-03 *default shape-mode* `ratelimit.exceeded` check selects the single most-recent row and asserts path includes `/auth/login`; a prior global-limiter flood (MAT-05) leaves global rows newest → **false FAIL**. Audit logging is correct & fully wired; **authoritative mode `CERT_AUDIT_RATELIMIT=1` PASSES 12/12**. No grading audit gap. Fix = select newest *login-limiter* row, or backlog (freeze policy) | **OPEN** — CTO decision (MAT-06) |
...
- [x] **CTO sign-off on MAT-05 (2026-06-28)** — **APPROVED**. Decision **PASS WITH NOTES** accepted: 10/10 scorecard, 0 Critical/High/Medium, targets comfortably exceeded, stress successful, teardown 0 residual, OBS-CW02-002 resolved as expected planner behaviour. **Rulings:** (1) **OBS-CW02-003** approved as a deferred Low-priority optimization — **do NOT implement during CW-02**; recorded as **MEB-002** (DB optimization candidate for future high-volume deployments). (2) **Baseline extension** approved in principle but **deferred** — do NOT modify the frozen Performance Framework during CW-02; recorded as the **Performance Baseline v2.0** post-CW-08 roadmap item. Frozen-platform policy maintained; no platform modifications authorized.
- [x] **MAT-05 gate CLOSED (2026-06-28)** — 0 open Critical/High. Next: **MAT-06 — Security & Reliability Certification**.
- [x] **MAT-06 executed (full batch, 2026-06-28)** — 14 security areas assessed against the grading surface; 3 frozen platform suites re-run live (**SS-04 config 31/3warn/0fail · SS-02 authz 235/235 · SS-03 audit authoritative 12/12 incl. immutability static+runtime**); 11 grading-specific probes **11/11 PASS** (config empty-body→400; negative-bound grade→400; blank gradedBy→400; **mass-assignment `grade:"A"` ignored, server computed `reject`**; mandatory `correctionReason` blank+missing→400; valid grade+correction; ECF genealogy seq1 original + seq2 correction preserved). Scanners: dep audit 0 high+, HoundDog 0, SAST 1 MEDIUM off-surface (mockup-sandbox dev tool, same as CW-01). Throwaway `CERTM06-*` fixtures torn down set-based → **0 residual** (baseline restored exactly: 58 cells / 14 lots / 1 ECF / 23 events). **Scorecard 10/10. 0 Critical/High/Medium.**
- [ ] **OBS-CW02-M06-001 (Low, Platform) — awaiting CTO ruling.** SS-03 *default shape-mode* `ratelimit.exceeded` check is order-dependent and false-FAILs on prior global-limiter flood residue (MAT-05); **authoritative mode `CERT_AUDIT_RATELIMIT=1` PASSES 12/12** and the audit logging is correct & fully wired (no grading audit gap). Recommend either (a) authorize a small SS-03 selector fix now (select newest *login-limiter* row / filter by limiter) — note the default-mode `audit` validation workflow stays red until then — or (b) backlog under the Platform Freeze Policy and rely on authoritative mode. **NO fix applied; awaiting CTO decision per batch methodology.**
- [ ] **MAT-06 gate — awaiting CTO sign-off** (Decision: **PASS WITH NOTE**; 0 Critical/High/Medium; 1 Low platform observation). Full record: `certification/CW-02-Cell-Grading/MAT-06.md`.
