...
- **Platforms are measured by ADOPTION, not new features.** Success = manufacturing capability delivered *on top of* the frozen platforms, not changes to the platforms themselves. Each frozen platform (ODS, ECF, Security Standards, Certification Framework) maintains a six-field scorecard in `docs/platform-scorecard.md`: Current Version · Freeze Status · Modules Using It · Last Platform Change · Next Planned Version · Enhancement Backlog Count. The headline metric is **Modules Using It** — grow adoption across manufacturing modules while Current Version stays put. Update the scorecard when a module adopts a platform, when a permitted (Critical/High/security) platform change ships, or at each cert-wave gate.
